...
| M4  | Fine-tune BERTweet                         | Co baseline Transformer toi uu cho text mang xa hoi | Script fine-tune/eval, checkpoint tot nhat, metrics theo cung schema                 | M2             | 1-2 ngay   | Cao        | Done (code + docs; can chay train de co artifact) |
| M5  | LLM prompting va danh gia                  | Danh gia zero/few-shot + kha nang giai thich        | Prompt templates, inference script, file du doan + rationale + cost/latency          | M2             | 0.5-1 ngay | Trung binh | Done (code + docs; can OPENAI_API_KEY de chay)    |
| M6  | Semantic analysis (STS + clustering)       | Do muc do hieu ngu nghia ngoai metrics classify     | STS report, embedding + UMAP/HDBSCAN clustering, hinh minh hoa                       | M3, M4, M5     | 1 ngay     | Cao        | Done (code + docs; can artifact M3/M4/M5 de chay) |
